using MangaViewer.Foundation.Converter;
using System;
...
namespace MangaViewer.Foundation.Controls
{
    ...
    public class HubTileHeightConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, string language)
        {
            string para = parameter as string;

            if (para.Equals("Negative"))
                return 0 - (double)value;
            else if (para.Equals("DoubleSize"))
                return 2 * (double)value;
            else if (para.Equals("HalfSize"))
                return 0 - 0.45 * (double)value;
            else
                return value;
        }
